...
private fun Preferences.toUserPreferencesEntity() = UserPreferencesEntity(
    id = 1,
...
private fun deleteLegacyDataStoreFiles(context: Context, dataStoreFile: File) {
...
suspend fun migrateFromDataStoreIfNeeded(context: Context, dao: CorePreferencesDao) {
    ...
    try {
        val entity = context.legacyDataStore.data.first().toUserPreferencesEntity()
        dao.upsert(entity)
